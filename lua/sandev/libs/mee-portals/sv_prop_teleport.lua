...
local allEnts
timer.Create("sev_portals_ent_update", 0.5, 0, function()
    local portals = ents.FindByClass("sev_portal")
    allEnts = ents.GetAll()
    ...
    for i = #allEnts, 1, -1 do 
        ...
        if not IsValid(prop) or
           not IsValid(prop:GetPhysicsObject()) or
           prop:GetVelocity() == Vector(0, 0, 0) or 
           prop:IsPlayer() or
           SEv.Ent:GetRealClass(prop) == "sev_portal"
            then
...
local function seamless_check(e)
    return SEv.Ent:GetRealClass(e) == "sev_portal" -- for traces
end
